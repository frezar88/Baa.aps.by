...
const TableLCV = () => {
    ...
    useEffect(() => {
        getAllStatisticModel().then(data => {
            setCarList(data.data.filter(item => item.car['car_type'].name === "LCV"))
            setLoad1(true)
        })
    }, [])
    ...
    useEffect(() => {
        getAllStatisticModelValue().then(data => {
            ...
            for (let dataKey in data.data) {
                data.data[dataKey].filter(item => item.car['car_type'].name === "LCV").forEach(el => {
                    array.push({
                        'date': dataKey,
                        'value': el.value,
                        'car_id': el['car']['id']
        ...
        <div style={{marginTop: 20, paddingBottom: 20}}>
            <div className={s.table}>
                <div style={{
                    ...
                    <h3 style={{paddingBottom: '10px', paddingLeft: `0 15`}}>Статистика LCV моделей</h3>
