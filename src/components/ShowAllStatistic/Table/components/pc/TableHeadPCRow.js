...
const TableHeadPCRow = ({stateYear}) => {
    ...
    const sort = (e, mouth, str) => {
        let block;
        if (!str) {
            block = document.querySelectorAll('div[data-mount-id="' + mouth + '"][data-pc-id]')
        } else {
    ...
    const highLightSortBlock = (mouth, e) => {
        let allBlockMonth = document.querySelectorAll('div[data-pc-id]')
        ...
        addHighLightArrow(e)
        if (mouth) {
            let block = document.querySelectorAll('div[data-mount-id="' + mouth + '"][data-pc-id]')
    ...
    const addHighLightArrow = (e) => {
        let allHeaderBlock = document.querySelectorAll('div.model_pc_top_block')
        if (allHeaderBlock[0]) {
