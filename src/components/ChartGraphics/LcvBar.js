        ...
        if (props.from && props.from !== 'Thu Jan 01 1970 03:00:00 GMT+0300 (Moscow Standard Time)') {
            try {
                getAllStatisticLcvOrPcForGraphicsMonth(props.from, props.to).then(data => {
                    ...
                    data.data.filter(item => item.car['car_type'].name === 'LCV').sort(function (a, b) {
                        return b.value - a.value
                    }).forEach(el => {
                        if (labels.length < 20) {
                            let sub = ''
                            if (el.car['car_subtype']) {
                                sub = el.car['car_subtype'].name
                            }
                ...
                <Bar className='p-1' height={200} options={{
                    ...
                    plugins: {
                        ...
                        legend: {
                            ...
                            title: {
                                display: true,
                                text: 'Топ 20 продаваемых коммерческих  моделей',
                                font: {
                                    size: 24
                                }
                            },
